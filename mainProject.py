...
def get_user_value():
    from projectClassUSD import USD  # importing our classes of currency
    ...
    if user_answer == '1':  # checking which option the user chose
        while True:  #making sure the user chose a valid answer
            try:
                ...
                # converting the amount shekel to dollar using the function from our class
            except ValueError:
...
def last_screen(results_list):
    ...
    print(results_list)
    result_file = open('results_list.txt', 'w+') #opening our file and writing our result list to the file
    result_file.write(str(results_list))
    result_file.close()
    return results_list
